...
// [[dust2::class(malariasimple_deterministic_ITN_IRS2)]]
...
// [[dust2::parameter(r_itn_daily_raw, type = "real_type", rank = 1, required = TRUE, constant = FALSE)]]
// [[dust2::parameter(s_itn_daily_raw, type = "real_type", rank = 1, required = TRUE, constant = FALSE)]]
...
class malariasimple_deterministic_ITN_IRS2 {
public:
  malariasimple_deterministic_ITN_IRS2() = delete;
  ...
  struct shared_state {
    ...
    struct dim_type {
      ...
      dust2::array::dimensions<1> r_itn_daily_raw;
      dust2::array::dimensions<1> s_itn_daily_raw;
    ...
    std::vector<real_type> r_itn_daily_raw;
    std::vector<real_type> s_itn_daily_raw;
  ...
  static shared_state build_shared(cpp11::list parameters) {
    ...
    dim.r_itn_daily_raw.set({static_cast<size_t>(n_days + 1)});
    dim.s_itn_daily_raw.set({static_cast<size_t>(n_days + 1)});
    ...
    std::vector<real_type> r_itn_daily_raw(dim.r_itn_daily_raw.size);
    dust2::r::read_real_array(parameters, dim.r_itn_daily_raw, r_itn_daily_raw.data(), "r_itn_daily_raw", true);
    std::vector<real_type> s_itn_daily_raw(dim.s_itn_daily_raw.size);
    dust2::r::read_real_array(parameters, dim.s_itn_daily_raw, s_itn_daily_raw.data(), "s_itn_daily_raw", true);
    ...
    const auto interpolate_r_itn = dust2::interpolate::InterpolateLinear(days, r_itn_daily_raw, "days", "r_itn_daily_raw");
    const auto interpolate_s_itn = dust2::interpolate::InterpolateLinear(days, s_itn_daily_raw, "days", "s_itn_daily_raw");
    ...
    return shared_state{odin, dim, n_days, na, nh, ft, eta, rA, rT, rD, rU, rP, dE, lag_rates, dCM, uCA, dCA, dB, uB, dID, uD, age20l, age20u, age_20_factor, PM, phi0, phi1, IC0, kC, b0, b1, kB, IB0, aD, fD0, gammaD, d1, ID0, kD, init_Sv, init_Pv, init_Iv, cU, cD, cT, gamma1, lag_ratesMos, FOIv_eq, omega, delayGam, delayMos, dLL, dPL, dEL, muLL, muEL, muPL, gammaL, mv0, mum, foraging_time, gonotrophic_cycle, betaL, init_PL, init_LL, init_EL, max_irs_cov, phi_indoors, max_itn_cov, Q0, phi_bednets, num_int, w1, z1, prev_dim, inc_dim, delayMos_use, mum_use, b_lambda, alpha_smc_times, alpha_smc_set, cov_, daily_rain_input, days, age_rate, het_wt, init_S, init_T, init_D, init_A, init_U, init_P, FOI_eq, psi, zeta, x_I, init_ICM, init_ICA, init_IB, init_ID, age_vector, p10, p2, interpolate_alpha_smc, P_smc_daily, smc_mask, rel_c_days, irs_eff_cov_daily, ss, rs, r_itn_daily_raw, s_itn_daily_raw, itn_eff_cov_daily, cov, min_age_prev, max_age_prev, min_age_inc, max_age_inc, fd, interpolate_rain_input, interpolate_P_smc, interpolate_rel_c, interpolate_s_irs, interpolate_r_irs, interpolate_irs_eff_cov, interpolate_r_itn, interpolate_s_itn, interpolate_itn_eff_cov};
  }
  static internal_state build_internal(const shared_state& shared) {
    ...
    return internal_state{S_death, TP_trans, T_death, DA_trans, D_death, AU_trans, A_death, US_trans, U_death, PS_trans, P_death, FOI, init_ICM_pre, IC, b, all, births, S_age, T_age, D_age, A_age, U_age, P_age, phi, n_prev, icm_pop, ica_pop, id_pop, ib_pop, ic_pop, all_deaths, epsilon_0_foi, p_det, alpha_smc_array, DS_trans, AS_trans, US_trans_SMC, cA, FOI_smc, smc_rel_c_mask, detect_prev_full, ST_rate, SD_rate, SA_rate, AT_rate, AD_rate, UA_rate, UD_rate, UT_rate, w_, z_, detect_prev, ST_trans, SD_trans, SA_trans, AT_trans, AD_trans, UA_trans, UD_trans, UT_trans, w, yy_, z, yy, zhi, whi, clin_inc, av_mosq, av_human, EIR, FOIvijk, FOI_lag, all_eir, epsilon_0, epsilon_0_no_int, epsilon_0_irs};
  }
  static void update_shared(cpp11::list parameters, shared_state& shared) {
    ...
    dust2::r::read_real_array(parameters, shared.dim.r_itn_daily_raw, shared.r_itn_daily_raw.data(), "r_itn_daily_raw", false);
    dust2::r::read_real_array(parameters, shared.dim.s_itn_daily_raw, shared.s_itn_daily_raw.data(), "s_itn_daily_raw", false);
    ...
    const auto interpolate_r_itn = dust2::interpolate::InterpolateLinear(shared.days, shared.r_itn_daily_raw, "days", "r_itn_daily_raw");
    const auto interpolate_s_itn = dust2::interpolate::InterpolateLinear(shared.days, shared.s_itn_daily_raw, "days", "s_itn_daily_raw");
  ...
  static void initial(real_type time, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state) {
    for (size_t i = 1; i <= shared.dim.S.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.S.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.S.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2] + 57] = shared.init_S[i - 1 + (j - 1) * shared.dim.init_S.mult[1] + (k - 1) * shared.dim.init_S.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.T.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.T.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.T.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.T.mult[1] + (k - 1) * shared.dim.T.mult[2] + shared.odin.offset.state[58]] = shared.init_T[i - 1 + (j - 1) * shared.dim.init_T.mult[1] + (k - 1) * shared.dim.init_T.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.D.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.D.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.D.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.D.mult[1] + (k - 1) * shared.dim.D.mult[2] + shared.odin.offset.state[59]] = shared.init_D[i - 1 + (j - 1) * shared.dim.init_D.mult[1] + (k - 1) * shared.dim.init_D.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.A.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.A.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.A.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.A.mult[1] + (k - 1) * shared.dim.A.mult[2] + shared.odin.offset.state[60]] = shared.init_A[i - 1 + (j - 1) * shared.dim.init_A.mult[1] + (k - 1) * shared.dim.init_A.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.U.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.U.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.U.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.U.mult[1] + (k - 1) * shared.dim.U.mult[2] + shared.odin.offset.state[61]] = shared.init_U[i - 1 + (j - 1) * shared.dim.init_U.mult[1] + (k - 1) * shared.dim.init_U.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.P.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.P.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.P.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.P.mult[1] + (k - 1) * shared.dim.P.mult[2] + shared.odin.offset.state[62]] = shared.init_P[i - 1 + (j - 1) * shared.dim.init_P.mult[1] + (k - 1) * shared.dim.init_P.mult[2]];
        }
      }
    }
      ...
      for (size_t j = 1; j <= shared.dim.FOI_XL.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.FOI_XL.dim[2]; ++k) {
          for (size_t l = 1; l <= shared.dim.FOI_XL.dim[3]; ++l) {
            state[i - 1 + (j - 1) * shared.dim.FOI_XL.mult[1] + (k - 1) * shared.dim.FOI_XL.mult[2] + (l - 1) * shared.dim.FOI_XL.mult[3] + shared.odin.offset.state[63]] = shared.FOI_eq[i - 1 + (j - 1) * shared.dim.FOI_eq.mult[1]];
          }
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.ICM.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.ICM.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.ICM.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.ICM.mult[1] + (k - 1) * shared.dim.ICM.mult[2] + shared.odin.offset.state[64]] = shared.init_ICM[i - 1 + (j - 1) * shared.dim.init_ICM.mult[1] + (k - 1) * shared.dim.init_ICM.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.ICA.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.ICA.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.ICA.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.ICA.mult[1] + (k - 1) * shared.dim.ICA.mult[2] + shared.odin.offset.state[65]] = shared.init_ICA[i - 1 + (j - 1) * shared.dim.init_ICA.mult[1] + (k - 1) * shared.dim.init_ICA.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.IB.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.IB.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.IB.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.IB.mult[1] + (k - 1) * shared.dim.IB.mult[2] + shared.odin.offset.state[66]] = shared.init_IB[i - 1 + (j - 1) * shared.dim.init_IB.mult[1] + (k - 1) * shared.dim.init_IB.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.ID.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.ID.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.ID.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.ID.mult[1] + (k - 1) * shared.dim.ID.mult[2] + shared.odin.offset.state[67]] = shared.init_ID[i - 1 + (j - 1) * shared.dim.init_ID.mult[1] + (k - 1) * shared.dim.init_ID.mult[2]];
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.FOIv.size; ++i) {
      state[i - 1 + shared.odin.offset.state[68]] = shared.FOIv_eq * shared.delayGam / shared.lag_ratesMos;
    }
    for (size_t i = 1; i <= shared.dim.ince_delay.size; ++i) {
      state[i - 1 + shared.odin.offset.state[69]] = shared.FOIv_eq * shared.init_Sv * shared.mv0 * shared.delayMos_use / shared.lag_ratesMos;
    }
    state[3] = 0;
    ...
    for (size_t i = 1; i <= shared.dim.n_ud_prev.size; ++i) {
      state[i - 1 + shared.odin.offset.state[70]] = shared.min_age_prev[i - 1];
    }
    for (size_t i = 1; i <= shared.dim.n_ud_detect_prev.size; ++i) {
      state[i - 1 + shared.odin.offset.state[71]] = shared.min_age_prev[i - 1];
    }
    for (size_t i = 1; i <= shared.dim.n_ud_inc.size; ++i) {
      state[i - 1 + shared.odin.offset.state[72]] = shared.min_age_inc[i - 1];
    }
    state[7] = 0;
    state[8] = 0;
    state[9] = 0;
  ...
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    const auto * S = state + 57;
    const auto * T = state + shared.odin.offset.state[58];
    const auto * D = state + shared.odin.offset.state[59];
    const auto * A = state + shared.odin.offset.state[60];
    const auto * U = state + shared.odin.offset.state[61];
    const auto * P = state + shared.odin.offset.state[62];
    const auto * FOI_XL = state + shared.odin.offset.state[63];
    const auto * ICM = state + shared.odin.offset.state[64];
    const auto * ICA = state + shared.odin.offset.state[65];
    const auto * IB = state + shared.odin.offset.state[66];
    const auto * ID = state + shared.odin.offset.state[67];
    const auto Sv = state[0];
    const auto Pv = state[1];
    const auto Iv = state[2];
    const auto * FOIv = state + shared.odin.offset.state[68];
    const auto * ince_delay = state + shared.odin.offset.state[69];
    ...
    const real_type KL = K0 * rain_input;
    {
      ...
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[(j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2] + 57] = S[i - 1 + (j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2]] + internal.PS_trans[i - 1 + (j - 1) * shared.dim.PS_trans.mult[1] + (k - 1) * shared.dim.PS_trans.mult[2]] + internal.US_trans[i - 1 + (j - 1) * shared.dim.US_trans.mult[1] + (k - 1) * shared.dim.US_trans.mult[2]] + internal.US_trans_SMC[i - 1 + (j - 1) * shared.dim.US_trans_SMC.mult[1] + (k - 1) * shared.dim.US_trans_SMC.mult[2]] + internal.AS_trans[i - 1 + (j - 1) * shared.dim.AS_trans.mult[1] + (k - 1) * shared.dim.AS_trans.mult[2]] + internal.DS_trans[i - 1 + (j - 1) * shared.dim.DS_trans.mult[1] + (k - 1) * shared.dim.DS_trans.mult[2]] - internal.ST_trans[i - 1 + (j - 1) * shared.dim.ST_trans.mult[1] + (k - 1) * shared.dim.ST_trans.mult[2]] - internal.SD_trans[i - 1 + (j - 1) * shared.dim.SD_trans.mult[1] + (k - 1) * shared.dim.SD_trans.mult[2]] - internal.SA_trans[i - 1 + (j - 1) * shared.dim.SA_trans.mult[1] + (k - 1) * shared.dim.SA_trans.mult[2]] - internal.S_death[i - 1 + (j - 1) * shared.dim.S_death.mult[1] + (k - 1) * shared.dim.S_death.mult[2]] - internal.S_age[i - 1 + (j - 1) * shared.dim.S_age.mult[1] + (k - 1) * shared.dim.S_age.mult[2]] + internal.births[(j - 1) * shared.dim.births.mult[1] + (k - 1) * shared.dim.births.mult[2]];
        }
      }
    }
    for (size_t i = 2; i <= static_cast<size_t>(shared.na); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2] + 57] = S[i - 1 + (j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2]] + internal.PS_trans[i - 1 + (j - 1) * shared.dim.PS_trans.mult[1] + (k - 1) * shared.dim.PS_trans.mult[2]] + internal.US_trans[i - 1 + (j - 1) * shared.dim.US_trans.mult[1] + (k - 1) * shared.dim.US_trans.mult[2]] + internal.US_trans_SMC[i - 1 + (j - 1) * shared.dim.US_trans_SMC.mult[1] + (k - 1) * shared.dim.US_trans_SMC.mult[2]] + internal.AS_trans[i - 1 + (j - 1) * shared.dim.AS_trans.mult[1] + (k - 1) * shared.dim.AS_trans.mult[2]] + internal.DS_trans[i - 1 + (j - 1) * shared.dim.DS_trans.mult[1] + (k - 1) * shared.dim.DS_trans.mult[2]] - internal.ST_trans[i - 1 + (j - 1) * shared.dim.ST_trans.mult[1] + (k - 1) * shared.dim.ST_trans.mult[2]] - internal.SD_trans[i - 1 + (j - 1) * shared.dim.SD_trans.mult[1] + (k - 1) * shared.dim.SD_trans.mult[2]] - internal.SA_trans[i - 1 + (j - 1) * shared.dim.SA_trans.mult[1] + (k - 1) * shared.dim.SA_trans.mult[2]] - internal.S_death[i - 1 + (j - 1) * shared.dim.S_death.mult[1] + (k - 1) * shared.dim.S_death.mult[2]] - internal.S_age[i - 1 + (j - 1) * shared.dim.S_age.mult[1] + (k - 1) * shared.dim.S_age.mult[2]] + internal.S_age[i - 1 - 1 + (j - 1) * shared.dim.S_age.mult[1] + (k - 1) * shared.dim.S_age.mult[2]];
        }
      }
    }
    {
    ...
    for (size_t i = 2; i <= static_cast<size_t>(shared.na); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.T.mult[1] + (k - 1) * shared.dim.T.mult[2] + shared.odin.offset.state[58]] = T[i - 1 + (j - 1) * shared.dim.T.mult[1] + (k - 1) * shared.dim.T.mult[2]] + internal.AT_trans[i - 1 + (j - 1) * shared.dim.AT_trans.mult[1] + (k - 1) * shared.dim.AT_trans.mult[2]] + internal.ST_trans[i - 1 + (j - 1) * shared.dim.ST_trans.mult[1] + (k - 1) * shared.dim.ST_trans.mult[2]] + internal.UT_trans[i - 1 + (j - 1) * shared.dim.UT_trans.mult[1] + (k - 1) * shared.dim.UT_trans.mult[2]] - internal.TP_trans[i - 1 + (j - 1) * shared.dim.TP_trans.mult[1] + (k - 1) * shared.dim.TP_trans.mult[2]] - internal.T_age[i - 1 + (j - 1) * shared.dim.T_age.mult[1] + (k - 1) * shared.dim.T_age.mult[2]] - internal.T_death[i - 1 + (j - 1) * shared.dim.T_death.mult[1] + (k - 1) * shared.dim.T_death.mult[2]] + internal.T_age[i - 1 - 1 + (j - 1) * shared.dim.T_age.mult[1] + (k - 1) * shared.dim.T_age.mult[2]];
        }
      }
    }
    {
      ...
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[(j - 1) * shared.dim.D.mult[1] + (k - 1) * shared.dim.D.mult[2] + shared.odin.offset.state[59]] = D[i - 1 + (j - 1) * shared.dim.D.mult[1] + (k - 1) * shared.dim.D.mult[2]] + internal.SD_trans[i - 1 + (j - 1) * shared.dim.SD_trans.mult[1] + (k - 1) * shared.dim.SD_trans.mult[2]] + internal.AD_trans[i - 1 + (j - 1) * shared.dim.AD_trans.mult[1] + (k - 1) * shared.dim.AD_trans.mult[2]] + internal.UD_trans[i - 1 + (j - 1) * shared.dim.UD_trans.mult[1] + (k - 1) * shared.dim.UD_trans.mult[2]] - internal.DA_trans[i - 1 + (j - 1) * shared.dim.DA_trans.mult[1] + (k - 1) * shared.dim.DA_trans.mult[2]] - internal.DS_trans[i - 1 + (j - 1) * shared.dim.DS_trans.mult[1] + (k - 1) * shared.dim.DS_trans.mult[2]] - internal.D_death[i - 1 + (j - 1) * shared.dim.D_death.mult[1] + (k - 1) * shared.dim.D_death.mult[2]] - internal.D_age[i - 1 + (j - 1) * shared.dim.D_age.mult[1] + (k - 1) * shared.dim.D_age.mult[2]];
        }
      }
    }
    for (size_t i = 2; i <= static_cast<size_t>(shared.na); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.D.mult[1] + (k - 1) * shared.dim.D.mult[2] + shared.odin.offset.state[59]] = D[i - 1 + (j - 1) * shared.dim.D.mult[1] + (k - 1) * shared.dim.D.mult[2]] + internal.SD_trans[i - 1 + (j - 1) * shared.dim.SD_trans.mult[1] + (k - 1) * shared.dim.SD_trans.mult[2]] + internal.AD_trans[i - 1 + (j - 1) * shared.dim.AD_trans.mult[1] + (k - 1) * shared.dim.AD_trans.mult[2]] + internal.UD_trans[i - 1 + (j - 1) * shared.dim.UD_trans.mult[1] + (k - 1) * shared.dim.UD_trans.mult[2]] - internal.DA_trans[i - 1 + (j - 1) * shared.dim.DA_trans.mult[1] + (k - 1) * shared.dim.DA_trans.mult[2]] - internal.DS_trans[i - 1 + (j - 1) * shared.dim.DS_trans.mult[1] + (k - 1) * shared.dim.DS_trans.mult[2]] - internal.D_death[i - 1 + (j - 1) * shared.dim.D_death.mult[1] + (k - 1) * shared.dim.D_death.mult[2]] - internal.D_age[i - 1 + (j - 1) * shared.dim.D_age.mult[1] + (k - 1) * shared.dim.D_age.mult[2]] + internal.D_age[i - 1 - 1 + (j - 1) * shared.dim.D_age.mult[1] + (k - 1) * shared.dim.D_age.mult[2]];
        }
      }
    }
    {
      ...
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[(j - 1) * shared.dim.A.mult[1] + (k - 1) * shared.dim.A.mult[2] + shared.odin.offset.state[60]] = A[i - 1 + (j - 1) * shared.dim.A.mult[1] + (k - 1) * shared.dim.A.mult[2]] + internal.SA_trans[i - 1 + (j - 1) * shared.dim.SA_trans.mult[1] + (k - 1) * shared.dim.SA_trans.mult[2]] + internal.DA_trans[i - 1 + (j - 1) * shared.dim.DA_trans.mult[1] + (k - 1) * shared.dim.DA_trans.mult[2]] + internal.UA_trans[i - 1 + (j - 1) * shared.dim.UA_trans.mult[1] + (k - 1) * shared.dim.UA_trans.mult[2]] - internal.AT_trans[i - 1 + (j - 1) * shared.dim.AT_trans.mult[1] + (k - 1) * shared.dim.AT_trans.mult[2]] - internal.AD_trans[i - 1 + (j - 1) * shared.dim.AD_trans.mult[1] + (k - 1) * shared.dim.AD_trans.mult[2]] - internal.AU_trans[i - 1 + (j - 1) * shared.dim.AU_trans.mult[1] + (k - 1) * shared.dim.AU_trans.mult[2]] - internal.AS_trans[i - 1 + (j - 1) * shared.dim.AS_trans.mult[1] + (k - 1) * shared.dim.AS_trans.mult[2]] - internal.A_death[i - 1 + (j - 1) * shared.dim.A_death.mult[1] + (k - 1) * shared.dim.A_death.mult[2]] - internal.A_age[i - 1 + (j - 1) * shared.dim.A_age.mult[1] + (k - 1) * shared.dim.A_age.mult[2]];
        }
      }
    }
    for (size_t i = 2; i <= static_cast<size_t>(shared.na); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.A.mult[1] + (k - 1) * shared.dim.A.mult[2] + shared.odin.offset.state[60]] = A[i - 1 + (j - 1) * shared.dim.A.mult[1] + (k - 1) * shared.dim.A.mult[2]] + internal.SA_trans[i - 1 + (j - 1) * shared.dim.SA_trans.mult[1] + (k - 1) * shared.dim.SA_trans.mult[2]] + internal.DA_trans[i - 1 + (j - 1) * shared.dim.DA_trans.mult[1] + (k - 1) * shared.dim.DA_trans.mult[2]] + internal.UA_trans[i - 1 + (j - 1) * shared.dim.UA_trans.mult[1] + (k - 1) * shared.dim.UA_trans.mult[2]] - internal.AT_trans[i - 1 + (j - 1) * shared.dim.AT_trans.mult[1] + (k - 1) * shared.dim.AT_trans.mult[2]] - internal.AD_trans[i - 1 + (j - 1) * shared.dim.AD_trans.mult[1] + (k - 1) * shared.dim.AD_trans.mult[2]] - internal.AU_trans[i - 1 + (j - 1) * shared.dim.AU_trans.mult[1] + (k - 1) * shared.dim.AU_trans.mult[2]] - internal.AS_trans[i - 1 + (j - 1) * shared.dim.AS_trans.mult[1] + (k - 1) * shared.dim.AS_trans.mult[2]] - internal.A_death[i - 1 + (j - 1) * shared.dim.A_death.mult[1] + (k - 1) * shared.dim.A_death.mult[2]] - internal.A_age[i - 1 + (j - 1) * shared.dim.A_age.mult[1] + (k - 1) * shared.dim.A_age.mult[2]] + internal.A_age[i - 1 - 1 + (j - 1) * shared.dim.A_age.mult[1] + (k - 1) * shared.dim.A_age.mult[2]];
        }
      }
    }
    {
      ...
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[(j - 1) * shared.dim.U.mult[1] + (k - 1) * shared.dim.U.mult[2] + shared.odin.offset.state[61]] = U[i - 1 + (j - 1) * shared.dim.U.mult[1] + (k - 1) * shared.dim.U.mult[2]] + internal.AU_trans[i - 1 + (j - 1) * shared.dim.AU_trans.mult[1] + (k - 1) * shared.dim.AU_trans.mult[2]] - internal.UD_trans[i - 1 + (j - 1) * shared.dim.UD_trans.mult[1] + (k - 1) * shared.dim.UD_trans.mult[2]] - internal.UT_trans[i - 1 + (j - 1) * shared.dim.UT_trans.mult[1] + (k - 1) * shared.dim.UT_trans.mult[2]] - internal.US_trans[i - 1 + (j - 1) * shared.dim.US_trans.mult[1] + (k - 1) * shared.dim.US_trans.mult[2]] - internal.US_trans_SMC[i - 1 + (j - 1) * shared.dim.US_trans_SMC.mult[1] + (k - 1) * shared.dim.US_trans_SMC.mult[2]] - internal.UA_trans[i - 1 + (j - 1) * shared.dim.UA_trans.mult[1] + (k - 1) * shared.dim.UA_trans.mult[2]] - internal.U_age[i - 1 + (j - 1) * shared.dim.U_age.mult[1] + (k - 1) * shared.dim.U_age.mult[2]] - internal.U_death[i - 1 + (j - 1) * shared.dim.U_death.mult[1] + (k - 1) * shared.dim.U_death.mult[2]];
        }
      }
    }
    for (size_t i = 2; i <= static_cast<size_t>(shared.na); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.U.mult[1] + (k - 1) * shared.dim.U.mult[2] + shared.odin.offset.state[61]] = U[i - 1 + (j - 1) * shared.dim.U.mult[1] + (k - 1) * shared.dim.U.mult[2]] + internal.AU_trans[i - 1 + (j - 1) * shared.dim.AU_trans.mult[1] + (k - 1) * shared.dim.AU_trans.mult[2]] - internal.UD_trans[i - 1 + (j - 1) * shared.dim.UD_trans.mult[1] + (k - 1) * shared.dim.UD_trans.mult[2]] - internal.UT_trans[i - 1 + (j - 1) * shared.dim.UT_trans.mult[1] + (k - 1) * shared.dim.UT_trans.mult[2]] - internal.US_trans[i - 1 + (j - 1) * shared.dim.US_trans.mult[1] + (k - 1) * shared.dim.US_trans.mult[2]] - internal.US_trans_SMC[i - 1 + (j - 1) * shared.dim.US_trans_SMC.mult[1] + (k - 1) * shared.dim.US_trans_SMC.mult[2]] - internal.UA_trans[i - 1 + (j - 1) * shared.dim.UA_trans.mult[1] + (k - 1) * shared.dim.UA_trans.mult[2]] - internal.U_age[i - 1 + (j - 1) * shared.dim.U_age.mult[1] + (k - 1) * shared.dim.U_age.mult[2]] - internal.U_death[i - 1 + (j - 1) * shared.dim.U_death.mult[1] + (k - 1) * shared.dim.U_death.mult[2]] + internal.U_age[i - 1 - 1 + (j - 1) * shared.dim.U_age.mult[1] + (k - 1) * shared.dim.U_age.mult[2]];
        }
      }
    }
    {
      ...
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[(j - 1) * shared.dim.P.mult[1] + (k - 1) * shared.dim.P.mult[2] + shared.odin.offset.state[62]] = P[i - 1 + (j - 1) * shared.dim.P.mult[1] + (k - 1) * shared.dim.P.mult[2]] + internal.TP_trans[i - 1 + (j - 1) * shared.dim.TP_trans.mult[1] + (k - 1) * shared.dim.TP_trans.mult[2]] - internal.PS_trans[i - 1 + (j - 1) * shared.dim.PS_trans.mult[1] + (k - 1) * shared.dim.PS_trans.mult[2]] - internal.P_death[i - 1 + (j - 1) * shared.dim.P_death.mult[1] + (k - 1) * shared.dim.P_death.mult[2]] - internal.P_age[i - 1 + (j - 1) * shared.dim.P_age.mult[1] + (k - 1) * shared.dim.P_age.mult[2]];
        }
      }
    }
    for (size_t i = 2; i <= static_cast<size_t>(shared.na); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.P.mult[1] + (k - 1) * shared.dim.P.mult[2] + shared.odin.offset.state[62]] = P[i - 1 + (j - 1) * shared.dim.P.mult[1] + (k - 1) * shared.dim.P.mult[2]] + internal.TP_trans[i - 1 + (j - 1) * shared.dim.TP_trans.mult[1] + (k - 1) * shared.dim.TP_trans.mult[2]] - internal.PS_trans[i - 1 + (j - 1) * shared.dim.PS_trans.mult[1] + (k - 1) * shared.dim.PS_trans.mult[2]] - internal.P_death[i - 1 + (j - 1) * shared.dim.P_death.mult[1] + (k - 1) * shared.dim.P_death.mult[2]] - internal.P_age[i - 1 + (j - 1) * shared.dim.P_age.mult[1] + (k - 1) * shared.dim.P_age.mult[2]] + internal.P_age[i - 1 - 1 + (j - 1) * shared.dim.P_age.mult[1] + (k - 1) * shared.dim.P_age.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.FOI_XL.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.FOI_XL.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.FOI_XL.dim[2]; ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.FOI_XL.mult[1] + (k - 1) * shared.dim.FOI_XL.mult[2] + shared.odin.offset.state[63]] = FOI_XL[i - 1 + (j - 1) * shared.dim.FOI_XL.mult[1] + (k - 1) * shared.dim.FOI_XL.mult[2]] + dt * ((shared.lag_rates / shared.dE) * internal.FOI_lag[i - 1 + (j - 1) * shared.dim.FOI_lag.mult[1] + (k - 1) * shared.dim.FOI_lag.mult[2]] - (shared.lag_rates / shared.dE) * FOI_XL[i - 1 + (j - 1) * shared.dim.FOI_XL.mult[1] + (k - 1) * shared.dim.FOI_XL.mult[2]]);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.FOI_XL.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.FOI_XL.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.FOI_XL.dim[2]; ++k) {
          for (size_t l = 2; l <= static_cast<size_t>(shared.lag_rates); ++l) {
            state_next[i - 1 + (j - 1) * shared.dim.FOI_XL.mult[1] + (k - 1) * shared.dim.FOI_XL.mult[2] + (l - 1) * shared.dim.FOI_XL.mult[3] + shared.odin.offset.state[63]] = FOI_XL[i - 1 + (j - 1) * shared.dim.FOI_XL.mult[1] + (k - 1) * shared.dim.FOI_XL.mult[2] + (l - 1) * shared.dim.FOI_XL.mult[3]] + dt * ((shared.lag_rates / shared.dE) * FOI_XL[i - 1 + (j - 1) * shared.dim.FOI_XL.mult[1] + (k - 1) * shared.dim.FOI_XL.mult[2] + (l - 1 - 1) * shared.dim.FOI_XL.mult[3]] - (shared.lag_rates / shared.dE) * FOI_XL[i - 1 + (j - 1) * shared.dim.FOI_XL.mult[1] + (k - 1) * shared.dim.FOI_XL.mult[2] + (l - 1) * shared.dim.FOI_XL.mult[3]]);
          }
        }
      }
    }
    {
      ...
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[(j - 1) * shared.dim.ICM.mult[1] + (k - 1) * shared.dim.ICM.mult[2] + shared.odin.offset.state[64]] = ICM[i - 1 + (j - 1) * shared.dim.ICM.mult[1] + (k - 1) * shared.dim.ICM.mult[2]] + dt * (-1 / shared.dCM * ICM[i - 1 + (j - 1) * shared.dim.ICM.mult[1] + (k - 1) * shared.dim.ICM.mult[2]] + (internal.init_ICM_pre[j - 1 + (k - 1) * shared.dim.init_ICM_pre.mult[1]] - ICM[i - 1 + (j - 1) * shared.dim.ICM.mult[1] + (k - 1) * shared.dim.ICM.mult[2]]) / shared.x_I[i - 1]);
        }
      }
    }
    for (size_t i = 2; i <= static_cast<size_t>(shared.na); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.ICM.mult[1] + (k - 1) * shared.dim.ICM.mult[2] + shared.odin.offset.state[64]] = ICM[i - 1 + (j - 1) * shared.dim.ICM.mult[1] + (k - 1) * shared.dim.ICM.mult[2]] + dt * (-1 / shared.dCM * ICM[i - 1 + (j - 1) * shared.dim.ICM.mult[1] + (k - 1) * shared.dim.ICM.mult[2]] - (ICM[i - 1 + (j - 1) * shared.dim.ICM.mult[1] + (k - 1) * shared.dim.ICM.mult[2]] - ICM[i - 1 - 1 + (j - 1) * shared.dim.ICM.mult[1] + (k - 1) * shared.dim.ICM.mult[2]]) / shared.x_I[i - 1]);
        }
      }
    }
    {
      ...
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[(j - 1) * shared.dim.ICA.mult[1] + (k - 1) * shared.dim.ICA.mult[2] + shared.odin.offset.state[65]] = ICA[i - 1 + (j - 1) * shared.dim.ICA.mult[1] + (k - 1) * shared.dim.ICA.mult[2]] + dt * (internal.FOI_smc[i - 1 + (j - 1) * shared.dim.FOI_smc.mult[1] + (k - 1) * shared.dim.FOI_smc.mult[2]] / (internal.FOI_smc[i - 1 + (j - 1) * shared.dim.FOI_smc.mult[1] + (k - 1) * shared.dim.FOI_smc.mult[2]] * shared.uCA + 1) - 1 / shared.dCA * ICA[i - 1 + (j - 1) * shared.dim.ICA.mult[1] + (k - 1) * shared.dim.ICA.mult[2]] - ICA[i - 1 + (j - 1) * shared.dim.ICA.mult[1] + (k - 1) * shared.dim.ICA.mult[2]] / shared.x_I[i - 1]);
        }
      }
    }
    for (size_t i = 2; i <= static_cast<size_t>(shared.na); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.ICA.mult[1] + (k - 1) * shared.dim.ICA.mult[2] + shared.odin.offset.state[65]] = ICA[i - 1 + (j - 1) * shared.dim.ICA.mult[1] + (k - 1) * shared.dim.ICA.mult[2]] + dt * (internal.FOI_smc[i - 1 + (j - 1) * shared.dim.FOI_smc.mult[1] + (k - 1) * shared.dim.FOI_smc.mult[2]] / (internal.FOI_smc[i - 1 + (j - 1) * shared.dim.FOI_smc.mult[1] + (k - 1) * shared.dim.FOI_smc.mult[2]] * shared.uCA + 1) - 1 / shared.dCA * ICA[i - 1 + (j - 1) * shared.dim.ICA.mult[1] + (k - 1) * shared.dim.ICA.mult[2]] - (ICA[i - 1 + (j - 1) * shared.dim.ICA.mult[1] + (k - 1) * shared.dim.ICA.mult[2]] - ICA[i - 1 - 1 + (j - 1) * shared.dim.ICA.mult[1] + (k - 1) * shared.dim.ICA.mult[2]]) / shared.x_I[i - 1]);
        }
      }
    }
    {
      ...
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[(j - 1) * shared.dim.IB.mult[1] + (k - 1) * shared.dim.IB.mult[2] + shared.odin.offset.state[66]] = IB[i - 1 + (j - 1) * shared.dim.IB.mult[1] + (k - 1) * shared.dim.IB.mult[2]] + dt * (internal.EIR[i - 1 + (j - 1) * shared.dim.EIR.mult[1] + (k - 1) * shared.dim.EIR.mult[2]] / (internal.EIR[i - 1 + (j - 1) * shared.dim.EIR.mult[1] + (k - 1) * shared.dim.EIR.mult[2]] * shared.uB + 1) - IB[i - 1 + (j - 1) * shared.dim.IB.mult[1] + (k - 1) * shared.dim.IB.mult[2]] / shared.dB - IB[i - 1 + (j - 1) * shared.dim.IB.mult[1] + (k - 1) * shared.dim.IB.mult[2]] / shared.x_I[i - 1]);
        }
      }
    }
    for (size_t i = 2; i <= static_cast<size_t>(shared.na); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.IB.mult[1] + (k - 1) * shared.dim.IB.mult[2] + shared.odin.offset.state[66]] = IB[i - 1 + (j - 1) * shared.dim.IB.mult[1] + (k - 1) * shared.dim.IB.mult[2]] + dt * (internal.EIR[i - 1 + (j - 1) * shared.dim.EIR.mult[1] + (k - 1) * shared.dim.EIR.mult[2]] / (internal.EIR[i - 1 + (j - 1) * shared.dim.EIR.mult[1] + (k - 1) * shared.dim.EIR.mult[2]] * shared.uB + 1) - IB[i - 1 + (j - 1) * shared.dim.IB.mult[1] + (k - 1) * shared.dim.IB.mult[2]] / shared.dB - (IB[i - 1 + (j - 1) * shared.dim.IB.mult[1] + (k - 1) * shared.dim.IB.mult[2]] - IB[i - 1 - 1 + (j - 1) * shared.dim.IB.mult[1] + (k - 1) * shared.dim.IB.mult[2]]) / shared.x_I[i - 1]);
        }
      }
    }
    {
      ...
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[(j - 1) * shared.dim.ID.mult[1] + (k - 1) * shared.dim.ID.mult[2] + shared.odin.offset.state[67]] = ID[i - 1 + (j - 1) * shared.dim.ID.mult[1] + (k - 1) * shared.dim.ID.mult[2]] + dt * (internal.FOI_smc[i - 1 + (j - 1) * shared.dim.FOI_smc.mult[1] + (k - 1) * shared.dim.FOI_smc.mult[2]] / (internal.FOI_smc[i - 1 + (j - 1) * shared.dim.FOI_smc.mult[1] + (k - 1) * shared.dim.FOI_smc.mult[2]] * shared.uD + 1) - ID[i - 1 + (j - 1) * shared.dim.ID.mult[1] + (k - 1) * shared.dim.ID.mult[2]] / shared.dID - ID[i - 1 + (j - 1) * shared.dim.ID.mult[1] + (k - 1) * shared.dim.ID.mult[2]] / shared.x_I[i - 1]);
        }
      }
    }
    for (size_t i = 2; i <= static_cast<size_t>(shared.na); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.nh); ++j) {
        for (size_t k = 1; k <= static_cast<size_t>(shared.num_int); ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.ID.mult[1] + (k - 1) * shared.dim.ID.mult[2] + shared.odin.offset.state[67]] = ID[i - 1 + (j - 1) * shared.dim.ID.mult[1] + (k - 1) * shared.dim.ID.mult[2]] + dt * (internal.FOI_smc[i - 1 + (j - 1) * shared.dim.FOI_smc.mult[1] + (k - 1) * shared.dim.FOI_smc.mult[2]] / (internal.FOI_smc[i - 1 + (j - 1) * shared.dim.FOI_smc.mult[1] + (k - 1) * shared.dim.FOI_smc.mult[2]] * shared.uD + 1) - ID[i - 1 + (j - 1) * shared.dim.ID.mult[1] + (k - 1) * shared.dim.ID.mult[2]] / shared.dID - (ID[i - 1 + (j - 1) * shared.dim.ID.mult[1] + (k - 1) * shared.dim.ID.mult[2]] - ID[i - 1 - 1 + (j - 1) * shared.dim.ID.mult[1] + (k - 1) * shared.dim.ID.mult[2]]) / shared.x_I[i - 1]);
        }
      }
    }
    state_next[shared.odin.offset.state[68]] = FOIv[0] + dt * (lag_FOIv - (shared.lag_ratesMos / shared.delayGam) * FOIv[0]);
    for (size_t i = 2; i <= static_cast<size_t>(shared.lag_ratesMos); ++i) {
      state_next[i - 1 + shared.odin.offset.state[68]] = FOIv[i - 1] + dt * ((shared.lag_ratesMos / shared.delayGam) * FOIv[i - 1 - 1] - (shared.lag_ratesMos / shared.delayGam) * FOIv[i - 1]);
    }
    state_next[shared.odin.offset.state[69]] = ince_delay[0] + dt * (ince - (shared.lag_ratesMos / shared.delayMos_use) * ince_delay[0]);
    for (size_t i = 2; i <= static_cast<size_t>(shared.lag_ratesMos); ++i) {
      state_next[i - 1 + shared.odin.offset.state[69]] = ince_delay[i - 1] + dt * ((shared.lag_ratesMos / shared.delayMos_use) * ince_delay[i - 1 - 1] - (shared.lag_ratesMos / shared.delayMos_use) * ince_delay[i - 1]);
    }
    ...
    for (size_t i = 1; i <= shared.dim.n_ud_prev.size; ++i) {
      state_next[i - 1 + shared.odin.offset.state[70]] = internal.n_prev[i - 1];
    }
    for (size_t i = 1; i <= shared.dim.n_ud_detect_prev.size; ++i) {
      state_next[i - 1 + shared.odin.offset.state[71]] = internal.detect_prev[i - 1];
    }
    for (size_t i = 1; i <= shared.dim.n_ud_inc.size; ++i) {
      state_next[i - 1 + shared.odin.offset.state[72]] = dust2::array::sum<real_type>(internal.clin_inc.data(), shared.dim.clin_inc, {shared.min_age_inc[i - 1] - 1, shared.max_age_inc[i - 1] - 1}, {0, shared.dim.clin_inc.dim[1] - 1}, {0, shared.dim.clin_inc.dim[2] - 1}) / dt;
    }
    state_next[7] = Sh;
    state_next[8] = Th;
    state_next[9] = Dh;
    ...
    state_next[19] = mu;
    state_next[20] = dust2::array::sum<real_type>(internal.all_eir.data(), shared.dim.all_eir);
    state_next[21] = dust2::array::sum<real_type>(internal.epsilon_0.data(), shared.dim.epsilon_0);
    state_next[22] = dust2::array::sum<real_type>(internal.epsilon_0_no_int.data(), shared.dim.epsilon_0_no_int);
    state_next[23] = dust2::array::sum<real_type>(internal.epsilon_0_irs.data(), shared.dim.epsilon_0_irs);
    state_next[24] = dust2::array::sum<real_type>(internal.av_human.data(), shared.dim.av_human);
    state_next[25] = av;
    state_next[26] = wh;
    state_next[27] = internal.yy_[0];
    state_next[28] = internal.yy_[1];
    state_next[29] = internal.yy_[2];
    state_next[30] = internal.w_[0];
    state_next[31] = internal.w_[1];
    state_next[32] = internal.w_[2];
    state_next[33] = internal.w_[3];
    state_next[34] = internal.z_[0];
    state_next[35] = internal.z_[1];
    state_next[36] = internal.z_[2];
    state_next[37] = internal.z_[3];
    state_next[38] = wbar;
    state_next[39] = zbar;
    state_next[40] = dust2::array::sum<real_type>(internal.epsilon_0_foi.data(), shared.dim.epsilon_0_foi);
    state_next[41] = FOIv[shared.lag_ratesMos - 1];
    state_next[42] = lag_FOIv;
    state_next[43] = dust2::array::sum<real_type>(internal.all.data(), shared.dim.all, {0, shared.dim.all.dim[0] - 1}, {0, shared.dim.all.dim[1] - 1}, {0, 0});
    state_next[44] = dust2::array::sum<real_type>(internal.all.data(), shared.dim.all, {0, shared.dim.all.dim[0] - 1}, {0, shared.dim.all.dim[1] - 1}, {1, 1});
    state_next[45] = dust2::array::sum<real_type>(internal.all.data(), shared.dim.all, {0, shared.dim.all.dim[0] - 1}, {0, shared.dim.all.dim[1] - 1}, {2, 2});
    state_next[46] = dust2::array::sum<real_type>(internal.all.data(), shared.dim.all, {0, shared.dim.all.dim[0] - 1}, {0, shared.dim.all.dim[1] - 1}, {3, 3});
    state_next[47] = shared.cov_[0];
    state_next[48] = shared.cov_[1];
    state_next[49] = shared.cov_[2];
    state_next[50] = shared.cov_[3];
    state_next[51] = dust2::array::sum<real_type>(internal.all.data(), shared.dim.all);
    state_next[52] = s_irs;
    state_next[53] = r_irs;
    state_next[54] = blood_meal_rate;
    state_next[55] = p1;
    state_next[56] = shared.p2;
  }
};
...
[[cpp11::register]]
SEXP dust2_system_malariasimple_deterministic_ITN_IRS2_alloc(cpp11::list r_pars, cpp11::sexp r_time, cpp11::list r_time_control, cpp11::sexp r_n_particles, cpp11::sexp r_n_groups, cpp11::sexp r_seed, cpp11::sexp r_deterministic, cpp11::sexp r_n_threads) {
  return dust2::r::dust2_discrete_alloc<malariasimple_deterministic_ITN_IRS2>(r_pars, r_time, r_time_control, r_n_particles, r_n_groups, r_seed, r_deterministic, r_n_threads);
}
[[cpp11::register]]
SEXP dust2_system_malariasimple_deterministic_ITN_IRS2_run_to_time(cpp11::sexp ptr, cpp11::sexp r_time) {
  return dust2::r::dust2_system_run_to_time<dust2::dust_discrete<malariasimple_deterministic_ITN_IRS2>>(ptr, r_time);
}

[[cpp11::register]]
SEXP dust2_system_malariasimple_deterministic_ITN_IRS2_state(cpp11::sexp ptr, cpp11::sexp r_index_state, cpp11::sexp r_index_particle, cpp11::sexp r_index_group, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_system_state<dust2::dust_discrete<malariasimple_deterministic_ITN_IRS2>>(ptr, r_index_state, r_index_particle, r_index_group, preserve_particle_dimension, preserve_group_dimension);
}

[[cpp11::register]]
SEXP dust2_system_malariasimple_deterministic_ITN_IRS2_time(cpp11::sexp ptr) {
  return dust2::r::dust2_system_time<dust2::dust_discrete<malariasimple_deterministic_ITN_IRS2>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_malariasimple_deterministic_ITN_IRS2_set_state_initial(cpp11::sexp ptr) {
  return dust2::r::dust2_system_set_state_initial<dust2::dust_discrete<malariasimple_deterministic_ITN_IRS2>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_malariasimple_deterministic_ITN_IRS2_set_state(cpp11::sexp ptr, cpp11::list r_state) {
  return dust2::r::dust2_system_set_state<dust2::dust_discrete<malariasimple_deterministic_ITN_IRS2>>(ptr, r_state);
}

[[cpp11::register]]
SEXP dust2_system_malariasimple_deterministic_ITN_IRS2_reorder(cpp11::sexp ptr, cpp11::integers r_index) {
  return dust2::r::dust2_system_reorder<dust2::dust_discrete<malariasimple_deterministic_ITN_IRS2>>(ptr, r_index);
}

[[cpp11::register]]
SEXP dust2_system_malariasimple_deterministic_ITN_IRS2_rng_state(cpp11::sexp ptr) {
  return dust2::r::dust2_system_rng_state<dust2::dust_discrete<malariasimple_deterministic_ITN_IRS2>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_malariasimple_deterministic_ITN_IRS2_set_rng_state(cpp11::sexp ptr, cpp11::sexp r_rng_state) {
  return dust2::r::dust2_system_set_rng_state<dust2::dust_discrete<malariasimple_deterministic_ITN_IRS2>>(ptr, r_rng_state);
}

[[cpp11::register]]
SEXP dust2_system_malariasimple_deterministic_ITN_IRS2_set_time(cpp11::sexp ptr, cpp11::sexp r_time) {
  return dust2::r::dust2_system_set_time<dust2::dust_discrete<malariasimple_deterministic_ITN_IRS2>>(ptr, r_time);
}

[[cpp11::register]]
SEXP dust2_system_malariasimple_deterministic_ITN_IRS2_update_pars(cpp11::sexp ptr, cpp11::list pars) {
  return dust2::r::dust2_system_update_pars<dust2::dust_discrete<malariasimple_deterministic_ITN_IRS2>>(ptr, pars);
}

[[cpp11::register]]
SEXP dust2_system_malariasimple_deterministic_ITN_IRS2_simulate(cpp11::sexp ptr, cpp11::sexp r_times, cpp11::sexp r_index_state, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_system_simulate<dust2::dust_discrete<malariasimple_deterministic_ITN_IRS2>>(ptr, r_times, r_index_state, preserve_particle_dimension, preserve_group_dimension);
}
