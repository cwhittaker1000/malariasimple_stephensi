...
phi_indoors <- parameter() # endophagy indoors
...
w1 <- 1
w2 <- 1 - phi_bednets + phi_bednets * s_itn
w3 <- 1 - phi_indoors + phi_indoors * (1 - r_irs) * s_irs
w4 <- 1 - phi_indoors + phi_bednets * (1 - r_irs) * s_itn * s_irs +
  (phi_indoors - phi_bednets) * (1 - r_irs) * s_irs

dim(w_) <- 4
w_[1] <- w1

w_[2] <- w1 * (1 - itn_eff_cov) +
  w2 * itn_eff_cov

w_[3] <- w1 * (1 - irs_eff_cov) +
  w3 * irs_eff_cov

w_[4] <- w1 * (1 - irs_eff_cov) * (1 - itn_eff_cov) +
  w2 * (1 - irs_eff_cov) * itn_eff_cov +
  w3 * irs_eff_cov * (1 - itn_eff_cov) +
  w4 * irs_eff_cov * itn_eff_cov
...
z1 <- 0
z2 <- phi_bednets * r_itn
z3 <- phi_indoors * r_irs
z4 <- phi_bednets * (1 - r_irs) * r_itn + phi_indoors * r_irs

dim(z_) <- 4
z_[1] <- z1
z_[2] <- z2 * itn_eff_cov
z_[3] <- z3 * irs_eff_cov
z_[4] <- z2 * (1 - irs_eff_cov) * itn_eff_cov +
  z3 * irs_eff_cov * (1 - itn_eff_cov) +
  z4 * irs_eff_cov * itn_eff_cov
z[] <- z_[i]
dim(z) <- num_int
